...
Objectives probreg::computeObjectiveFunction(const std::vector<Vector3>& mu1, const std::vector<Matrix3>& sigma1,
                                             ...
                                             const Vector6& p,
                                             Float outlier_ratio, Float resolution) {
    const Float gauss_c1 = 10.0 * (1 - outlier_ratio);
    const Float gauss_c2 = outlier_ratio / pow(resolution, 3);
    const Float gauss_d3 = -std::log(gauss_c2);
    const Float lfd1 = -std::log(gauss_c1 + gauss_c2) - gauss_d3;
    const Float lfd2 = -2 * std::log((-std::log(gauss_c1 * std::exp(-0.5) + gauss_c2) - gauss_d3) / lfd1);
